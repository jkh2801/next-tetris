...
export const NextBox: FC<PropType> = ({ playerController }) => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      canvasController.current.matrix = createMatrix(4, 19);
  ...
  const draw = () => {
    ...
    const { nextPieces } = playerController.current;
    if (ctx) {
      ctx.fillStyle = "#000";
      ctx.fillRect(0, 0, width, height);
      drawMatrix(nextPieces[0], { x: 1, y: 1 });
      drawMatrix(nextPieces[1], { x: 1, y: 6 });
      drawMatrix(nextPieces[2], { x: 1, y: 11 });
  ...
  return (
    <div className="bg-black">
      <canvas width={92} height={432} ref={canvasRef}></canvas>
    </div>
  );
};
